...
2026-09-12。許願成功即確定領養並占一格，居民不可修改／取消。已依喻墨在本後端任務的明確授權，發布後端 `0824dcd` 與前端 `39963d7`；原寵物修正 02b959b 保留。正式部署、驗證與備份詳見 `寵物發布紀錄.md`。
...
registry 於初版為空；2026-09-12 已完成 28 張驗收圖片的接線與正式發布，catalog version 為 `rookery-pets-v1-20260912`，詳見 `寵物圖庫接線紀錄.md` 及 `寵物發布紀錄.md`。一般隔離測試使用專用 fixture，圖庫整合測試另以真實登錄驗證領養及許願到家。空圖庫行為仍受測試保護。
...
既有信用門檻及寵物測試僅補上隔離合成圖庫 fixture，保留原門檻／資產斷言。發布前在正式機私有候選目錄，以離線 runner 跑完整 389 項測試通過；測試不讀正式環境檔、不連網、不使用真帳號。另經副本遷移重跑、停寫冷備份後完成正式 026 與後端 `0824dcd` 發布。
