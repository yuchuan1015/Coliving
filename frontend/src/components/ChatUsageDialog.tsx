...
const tokens = (value: number | null) => value === null ? "未取得" : value.toLocaleString("zh-TW");
...
export function UsageSummary({ usage }: { usage: ChatUsage }) {
  return <>
    ...
    <section className="chat-usage-metric" aria-labelledby="usage-context-title">
      <h3 id="usage-context-title">目前上下文</h3>
      <p className="chat-usage-number">{tokens(usage.current_context_tokens)}{usage.current_context_tokens !== null && <small> tokens</small>}</p>
      <p>最近一次模型呼叫的輸入量，不是整段對話累計，也不是容量百分比。</p>
    </section>
    <UsageTotalCard title="本次消耗" description="最近一則回覆，包含工具迴圈的每一次呼叫。" totals={usage.this_reply} priceKnown={usage.price_known} />
    <UsageTotalCard title="整窗累計" description="這段對話至今的消耗；刪除訊息不會倒扣已花掉的用量。" totals={usage.conversation_total} priceKnown={usage.price_known} />
    <UsageTotalCard title="本月累計" description="此室友本月所有已記錄呼叫，依伺服器 UTC 月份統計。" totals={usage.this_month} priceKnown={usage.price_known} />
    <footer className="chat-usage-note">
      <p>費用僅為 USD 估算，以供應商帳單為準。未回報的數字不算成 0；快取與思考若有回報，已含於輸入／輸出，不另加總。</p>
      {usage.price_known && <p>後端單價表日期：{usage.prices_as_of || "未提供"}</p>}
      <p>此面板不會持續自動更新，需要時可按「重新讀取」。</p>
    </footer>
  </>;
}
...
export function UsageTotalCard({ title, description, totals, priceKnown }: { title: string; description: string; totals: UsageTotals | null; priceKnown: boolean }) {
  return <section className="chat-usage-metric" aria-label={title}>
    ...
    {totals?.usage_partial && <p className="chat-usage-warning">用量不完整 · 已知小計（{tokens(totals.missing_usage)} 次呼叫未拿全）</p>}
    <p>{description}</p>
    {totals?.calls === 0 && <p>尚無已記錄的呼叫；不是零消耗的保證。</p>}
    <dl className="chat-usage-details">
      <div><dt>輸入 tokens</dt><dd>{tokens(totals?.input_tokens ?? null)}</dd></div>
      <div><dt>輸出 tokens</dt><dd>{tokens(totals?.output_tokens ?? null)}</dd></div>
      <div><dt>呼叫次數</dt><dd>{tokens(totals?.calls ?? null)}</dd></div>
    </dl>
    <div className="chat-usage-cost">
      <span>估算費用</span>
      {!priceKnown ? <span>模型單價未確認，暫不估算</span> : <>
        <strong>{totals?.cost_usd == null ? "未取得" : `USD $${dollars(totals.cost_usd)}`}</strong>
        {totals?.cost_partial && <span className="chat-usage-warning">費用不完整{totals.cost_usd !== null ? " · 已知費用小計" : ""}</span>}
      </>}
    </div>
  </section>;
}
...
export function ChatUsageDialog({ agentId, revision, onClose }: { agentId: string; revision: number; onClose: () => void }) {
  ...
  return <dialog ref={dialog} className="chat-usage-dialog" aria-labelledby="chat-usage-title" onCancel={event => { event.preventDefault(); onClose(); }} onClose={onClose} onClick={event => {
    ...
    <header><div><span className="chat-usage-eyebrow">CONVERSATION USAGE</span><h2 id="chat-usage-title">用量與費用</h2></div><button type="button" onClick={onClose} aria-label="關閉用量面板" autoFocus>×</button></header>
    <div className="chat-usage-body" aria-busy={loading}>
      {loading ? <p className="chat-usage-state" role="status">正在讀取用量…</p> : error ? <p className="chat-usage-state" role="alert">{error}</p> : usage && <UsageSummary usage={usage} />}
    </div>
    <div className="chat-usage-actions"><button type="button" disabled={loading} onClick={() => setRetry(value => value + 1)}>重新讀取</button></div>
  </dialog>;
}
