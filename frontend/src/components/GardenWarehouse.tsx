...
import { saleQuantityLabel, validSaleQuantity, type GardenMarketGateway } from "../api/garden-market";
...
import { privateCropIcons } from "../data/private-crop-icons";
...
export function GardenWarehouse({ userId, active, locked, gateway, onSold, inventory, loadInventory }: Props) {
  ...
  const order = market.selected && !market.pending && <form className="market-order" onSubmit={e => { e.preventDefault(); void market.requestQuote(); }} aria-label={uiText("出售收成")}>
          ...
          <div className="private-actions"><button className="private-button" type="button" disabled={blocked} onClick={() => market.changeQuantity(market.selected!.quantity_g)}>{uiText("全部數量")}</button><button className="private-button" type="button" disabled={blocked} onClick={() => market.changeQuantity("")}>{uiText("部分數量")}</button></div>
          ...
          <p id="market-quantity-help" className="private-muted">{uiText("可填整數、小數或分數，例如 100、0.5、200/3，最多 128 字元；不使用畫面四捨五入的重量。")}</p>
          {!!market.quantity && !validSaleQuantity(market.quantity, market.selected.quantity_g) && <p className="private-error">{uiText("請填大於 0、且不超過庫存的有效克數。")}</p>}
  ...
  return <div className="garden-market">
    ...
    {market.pending && !market.denied && <section className="private-panel market-pending" aria-label={uiText("未確認的出售")}>
      ...
      <p className="private-muted">{uiText("作物：")}{market.pending.crop_id} · {exactQuantity(market.pending.quantity_g)}</p>
    ...
    {!market.denied && <div className="private-warehouses">{(["user", "agent"] as const).map(owner => {
      ...
      return <section className="private-panel" key={owner} aria-label={owner === "user" ? uiText("我的倉庫") : uiText("室友的倉庫")}>
        ...
        {rows.length ? <ul className="market-items">{rows.map(item => {
          ...
          const sellable = owner === "user" && data?.can_sell && price?.can_sell && validSaleQuantity(item.quantity_g, item.quantity_g);
          return <li key={item.crop_id}><div className="market-item-heading"><h3><span aria-hidden="true">{privateCropIcons[item.crop_id] ?? "🌱"}</span> {uiText(item.crop_name)}</h3><strong>{exactQuantity(item.quantity_g)}</strong></div>
            ...
            {owner === "user" && price?.can_sell && !validSaleQuantity(item.quantity_g, item.quantity_g) && <p className="private-muted">{uiText("此精確數量過長，暫時無法整份出售；請聯絡客服協助，不會截斷數量。")}</p>}
        ...
        })}</ul> : (data || fallback.data) && <div className="private-empty-small">{uiText("第一份收成，還在路上。")}</div>}
