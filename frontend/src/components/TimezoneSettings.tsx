...
export function TimezoneSettings({ onBusyChange }: { onBusyChange?: (busy: boolean) => void }) {
  ...
  }}><label htmlFor={id}>{uiText("艙室與排程的時區")}</label><select id={id} value={zone} disabled={busy} onChange={e => setZone(e.target.value)}>{zones.map(z => <option key={z} value={z}>{z}</option>)}</select><small>{uiText("排程依此時區執行；公共場域仍使用台北時間。修改會影響既有排程的執行時間。")}</small><button type="submit" disabled={busy}>{uiText("保存時區")}</button><p role="status">{uiText(notice)}</p></form>;
}
