...
import { WardrobeActions, DiningActions } from "./FurnitureActions";
import { PetActions } from "./PetActions";
import type { PetGateway } from "../api/pets";
...
export function CabinPanelDialog({ panel, summary, now, onClose, onRefreshWeather, petGateway }: { panel: CabinPanel; summary: FurnitureSummary | null; now: Date; onClose: () => void; onRefreshWeather: () => Promise<void>; petGateway?: PetGateway }) {
  useUiLanguage();
  ...
  return <dialog ref={dialog} className={`cabin-panel${panel === "pet" ? " pet-dialog" : ""}`} aria-labelledby="cabin-panel-title" onCancel={event => { event.preventDefault(); close(); }} onClose={close} onClick={event => { if (event.target === event.currentTarget) { const rect = event.currentTarget.getBoundingClientRect(); if (event.clientX < rect.left || event.clientX > rect.right || event.clientY < rect.top || event.clientY > rect.bottom) close(); } }}>
    ...
    {["wardrobe", "dining", "pet"].includes(panel) && <fieldset className="cabin-action-fields" disabled={cityBusy}>
      ...
      {panel === "pet" && <PetActions key={user?.id ?? "signed-out"} userId={user?.id ?? ""} onBusyChange={setCityBusy} gateway={petGateway} />}
    </fieldset>}
  </dialog>;
}
