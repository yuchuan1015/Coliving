...
const icons: Record<string, string> = { broccoli: "🥦", carrot: "🥕", cauliflower: "🥦", cucumber: "🥒", daikon: "🌱", kohlrabi: "🌱", pak_choi: "🥬", petite_oyster_mushroom: "🍄", potato: "🥔", tomato: "🍅", vegetable_fern: "🌿", water_spinach: "🌿" };
...
export function PrivateGardenPage({ gateway }: { gateway?: PrivateGardenGateway } = {}) {
  ...
  return <main className="private-garden-page">
    <div className="private-wrap"><header className="private-header"><Link to="/">{uiText("← 返回艙室")}</Link><span>CABIN / GARDEN</span></header>
      ...
      {data && tab === "inventory" && <><div className="private-section-heading"><h2>{uiText("各自收藏生活裡的收成")}</h2></div><p className="private-muted">{uiText("私人田地與公共農田分配的收成，都會記入各自的倉庫。")}</p><div className="private-warehouses">{(["user", "agent"] as const).map(owner => { const store = garden.inventory[owner]; return <section className="private-panel" key={owner}><div className="private-section-heading"><h2>{owner === "user" ? uiText("我的倉庫") : uiText("室友的倉庫")}</h2><span className="private-badge">{owner === "user" ? "YOU" : "AGENT"}</span></div><p className="private-muted">{owner === "user" ? uiText("你偷到的菜與分到的公共收成。") : uiText("室友採收的菜與分到的公共收成。")}</p>{store.error && <p role="alert" className="private-error">{store.error}</p>}{store.data && (store.data.items.length ? <ul className="private-inventory-list">{store.data.items.map(item => <li key={item.crop_id}><span><span aria-hidden="true">{icons[item.crop_id] ?? "🌱"}</span> {uiText(item.crop_name)}</span><strong>{quantity(item.quantity_g)}</strong></li>)}</ul> : <div className="private-empty-small">{uiText("第一份收成，還在路上。")}</div>)}{store.data?.has_more && <button className="private-button" type="button" disabled={locked || store.loading} onClick={() => void garden.loadMore(owner)}>{store.loading ? uiText("讀取中…") : uiText("載入更多收成")}</button>}</section>; })}</div><p className="private-fineprint">{uiText("畫面顯示值可能四捨五入；分數會另列精確數量，不影響實際庫存。")}</p></>}
