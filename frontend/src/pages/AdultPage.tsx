...
export function AdultPage() {
  ...
  const [data, setData] = useState<AdultResponse | null>(null);
  ...
  // form
  const [formCat, setFormCat] = useState("communication");
  ...
  const load = async (c?: string | null) => {
    try {
      const res = await getAdult(c || undefined);
      setData(res);
    } catch {
      setError("載入失敗");
    } finally {
  ...
  const handleSubmit = async () => {
    ...
    setError("");
    try {
      await submitArticle({ category: formCat, title: title.trim(), content: content.trim() });
      setTitle(""); setContent("");
      setShowForm(false);
      load(category);
    } catch {
