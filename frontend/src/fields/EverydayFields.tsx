...
export function MailField() {
  ...
  async function send(data: FormData) {
    ...
    if (kind === "timed") {
      ...
      // Send UTC: current backend normalizes with replace(tzinfo=UTC).
      payload = { ...payload, deliver_at: date.toISOString() };
    }
    const result = await api.post<MailOut>(`/mail/${kind}`, payload);
    setMessage(kind === "timed" ? `定時信已建立（${result.data.id}），預計 ${fieldTime(result.data.deliver_at)} 送達。送達前列表暫不提供追蹤。` : kind === "physical" ? "實體寄送訂單已建立；後續狀態以管理員更新為準。" : `信件已交給郵驛，預計 ${fieldTime(result.data.deliver_at)} 送達。`);
  }
  return <FieldFrame id="mail"><FieldTabs options={{ inbox: "收件匣", sent: "寄件匣", timed: "已送達定時信", physical: "實體寄送" }} value={tab} onChange={value => { setTab(value); setSelected(null); }} />{message && <p role="status">{message}</p>}<FieldPanel title="郵驛" action={<button onClick={() => setCompose(true)}>寫一封信</button>}><small>{unread.data ? `${unread.data.count} 封未讀` : "未讀數尚未同步"} · 最多顯示最近 100 封</small><ResourceState resource={unread} /><ResourceState resource={list} empty={list.data?.length === 0} /><div className="field-list">{list.data?.map(m => <article className="field-item" key={m.id}><div className="field-row"><h3>{!m.is_read && tab !== "sent" ? "● " : ""}{m.subject}</h3><span className="field-tag">{MAIL_TYPE_LABELS[m.mail_type] ?? m.mail_type}</span></div><p>{m.from_name ?? "系統"} → {m.to_name}</p><small>{fieldTime(m.created_at)}{m.deliver_at && ` · 送達 ${fieldTime(m.deliver_at)}`}</small>{m.status && <p>{STATUS_LABELS[m.status] ?? m.status}</p>}<button onClick={() => setSelected(m.id)}>閱讀信件</button>{(tab !== "sent" || user?.role === "admin") && <ConfirmAction title="刪除這封信" label="確認刪除" action={() => api.delete(`/mail/${encodeURIComponent(m.id)}`)} onDone={() => { setMessage("信件已刪除。"); changed(); }} />}</article>)}</div></FieldPanel>
    ...
    {compose && <FieldDialog title="寄送信件" onClose={() => setCompose(false)}><FieldTabs options={{ letter: "一般信件", timed: "定時投遞", physical: "實體寄送" }} value={kind} onChange={setKind} /><ResourceState resource={residents} /><FieldForm key={kind} label="確認寄送" submit={send} onDone={() => { setCompose(false); list.refresh(); unread.refresh(); }}><FieldInput name="subject" label="主旨" max={100} />{kind !== "physical" && <FieldSelect name="to_agent_id" label="收件室友" options={{ "": "請選擇收件人", ...recipients }} />}<FieldText />{kind === "letter" && <><label className="field-check"><input name="anonymous" type="checkbox" />匿名寄送</label><small>一般信件約 12～48 小時送達，不是即時訊息。</small></>}{kind === "timed" && <><FieldInput name="deliver_at" label="送達時間（Asia/Taipei）" type="datetime-local" /><small>建立後以系統名義寄送；目前 API 不提供送達前的寄件追蹤。</small></>}{kind === "physical" && <small>建立實體寄送需求，不代表已付款或已完成物流寄送。請勿在此填寫不必要的敏感資料。</small>}</FieldForm></FieldDialog>}
  </FieldFrame>;
}
