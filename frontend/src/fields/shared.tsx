...
export function FieldTabs({ options, value, onChange }: { options: Record<string, string>; value: string; onChange: (value: string) => void }) {
  useUiLanguage(); const { busy } = useContext(DialogBusy); return <nav className="field-tabs" aria-label={uiText("內容分類")}>{Object.entries(options).map(([key, label]) => <button key={key} disabled={busy} aria-pressed={key === value} onClick={() => onChange(key)}>{uiText(label)}</button>)}</nav>; }
export function FieldError({ error, retry }: { error?: ShelfError; retry?: () => void }) {
  useUiLanguage(); return error ? <div className="field-notice" role="alert"><p>{uiText(error.message)}</p>{error.status === 403 && /出生|年齡/.test(error.message) && <Link className="field-button" to="/settings">{uiText("查看出生年設定")}</Link>}{retry && <button onClick={retry}>{uiText("重新讀取")}</button>}</div> : null; }
export function ResourceState({ resource, empty }: { resource: { loading: boolean; error?: ShelfError; refresh: () => void }; empty?: boolean }) {
  useUiLanguage(); return <>{resource.loading && <p role="status">{uiText("正在讀取…")}</p>}<FieldError error={resource.error} retry={resource.refresh} />{!resource.loading && !resource.error && empty && <p className="field-empty">{uiText("這裡還沒有內容。")}</p>}</>; }
...
export function FieldDialog({ title, onClose, children }: { title: string; onClose: () => void; children: ReactNode }) {
  ...
  return <dialog ref={ref} className="field-dialog field-app" aria-labelledby={id} onCancel={e => { e.preventDefault(); if (!busy) onClose(); }}><header className="field-row"><h2 id={id}>{title}</h2><button type="button" disabled={busy} onClick={onClose} aria-label={uiText("關閉")}>×</button></header><DialogBusy.Provider value={{ busy, setBusy }}><fieldset disabled={busy} style={{ border: 0, padding: 0, margin: 0, minWidth: 0 }}>{children}</fieldset></DialogBusy.Provider></dialog>;
}
export function FieldForm({ submit, children, label = uiText("保存"), onDone, guarded = false, onBusyChange }: { submit: (data: FormData) => Promise<unknown>; children: ReactNode; label?: string; onDone: () => void; guarded?: boolean; onBusyChange?: (busy: boolean) => void }) {
  useUiLanguage();
  ...
  return <form onSubmit={send} className="field-form"><fieldset disabled={busy}>{children}<FieldError error={error} />{uncertain && <p role="alert">{uiText("尚未確認是否已送達，請關閉後重新讀取狀態再決定是否重試，避免重複送出。")}</p>}<button type="submit" className="primary" disabled={uncertain}>{busy ? uiText("處理中…") : label}</button></fieldset></form>;
}
...
export function ConfirmAction({ title, action, onDone, label = uiText("確認") }: { title: string; action: () => Promise<unknown>; onDone: () => void; label?: string }) {
  ...
  return <><button onClick={() => setOpen(true)}>{title}</button>{open && <FieldDialog title={title} onClose={() => setOpen(false)}><FieldForm label={label} submit={action} onDone={() => { setOpen(false); onDone(); }}><p>{uiText("確認要")}{title}{uiText("嗎？此操作會更新實際資料。")}</p></FieldForm></FieldDialog>}</>;
}
