...
export function FieldFrame({ id, children }: { id: FieldId; children: ReactNode }) {
  ...
  return <main className="field-app"><div className="field-shell">
    ...
    {children}<footer><Link to="/outside">← 選擇其他目的地</Link><span>社區時間 · Asia/Taipei</span></footer>
  </div></main>;
}
...
export function FieldForm({ submit, children, label = "保存", onDone }: { submit: (data: FormData) => Promise<unknown>; children: ReactNode; label?: string; onDone: () => void }) {
  ...
  async function send(e: FormEvent<HTMLFormElement>) {
    ...
    const payload = new FormData(e.currentTarget); lock.current = true; setBusy(true); dialogBusy(true); setError(undefined);
    try { await submit(payload); onDone(); }
    catch (err) { setError(shelfError(err)); }
    finally { lock.current = false; setBusy(false); dialogBusy(false); }
  }
  return <form onSubmit={send} className="field-form"><fieldset disabled={busy}>{children}<FieldError error={error} /><button type="submit" className="primary">{busy ? "處理中…" : label}</button></fieldset></form>;
}
