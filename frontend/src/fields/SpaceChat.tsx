...
export function SpaceChatContent({ space, onBusyChange }: { space: ChatSpace; onBusyChange?: (busy: boolean) => void }) {
  ...
  return <div className="field-stack">
    <div className="field-actions"><button disabled={sending || exporting} onClick={refresh}>更新聊天與名單</button><button disabled={!ready || exporting || sending} onClick={() => void exportChat()}>{exporting ? "準備中…" : "匯出帶走"}</button></div>
    ...
    {ready && people.data && <FieldForm guarded key={revision} label="送出訊息" onBusyChange={busy => { setSending(busy); onBusyChange?.(busy); }} submit={async data => {
