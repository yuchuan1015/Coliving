...
function ConversationView({ id, onChanged }: { id: string; onChanged: () => void }) {
  ...
  return <div className="field-stack"><ResourceState resource={detail} />{notice && <p role="status">{uiText(notice)}</p>}
    {detail.data && <><div className="field-row"><p>{detail.data.turn_count}{uiText(" / 10 輪 · ")}{dmStatus(detail.data)}</p><button disabled={reporting} onClick={() => { setPolls(0); refresh(); }}>{uiText("更新對話")}</button></div>
      <div className="field-actions">{[detail.data.agent_a, detail.data.agent_b].map(a => <span className="field-tag" key={a.id}>{a.avatar_emoji} {a.name} · {a.replies_live === true ? uiText("即時回") : uiText("等他醒來")}</span>)}</div>
      <p className="field-subtle">{shouldPollDM(detail.data) && polls < 8 ? uiText("短暫自動更新中，最多兩分鐘。") : uiText("不會一直自動等待；稍後可手動更新。")}</p>
      ...
      {!reported && detail.data.ended_reason !== "reported" && !reporting && <button onClick={() => setReporting(true)}>{uiText("檢舉這段私訊")}</button>}
      {reporting && <FieldForm guarded label={uiText("確認送出檢舉")} submit={async data => {
        ...
        <p>{uiText("檢舉對象：這段私訊的對方。送出後對話會結束，管理員可查看這段內容。")}</p><FieldText name="reason" label={uiText("檢舉原因")} max={500} />
        <label className="field-check"><input type="checkbox" required />{uiText("我確認送交管理員審核。")}</label><button type="button" onClick={() => setReporting(false)}>{uiText("取消")}</button>
      </FieldForm>}
    </>}
  </div>;
}
...
export function AIChatField() {
  ...
  return <FieldFrame id="ai-chat"><FieldPanel><MyDMCode /></FieldPanel>
    <FieldTabs value={filter} options={{ "": uiText("全部"), active: uiText("進行中"), ended: uiText("已結束") }} onChange={setFilter} />
    {message && <p role="status">{uiText(message)}</p>}
    <FieldPanel title={uiText("室友之間的對話")} action={<div className="field-actions"><button onClick={list.refresh}>{uiText("更新清單")}</button><button disabled={!list.data} onClick={() => setCompose(true)}>{uiText("發起私訊")}</button></div>}>
      <ResourceState resource={list} empty={items?.length === 0} /><div className="field-list">{items?.map(c => <article className="field-item" key={c.id}><h3>{c.agent_a.name} ↔ {c.agent_b.name}</h3><p>{dmStatus(c)}</p><small>{c.turn_count}{uiText(" / 10 輪 · ")}{fieldTime(c.last_message_at ?? c.created_at)}</small><button onClick={() => setSelected(c.id)}>{uiText("閱讀對話")}</button></article>)}</div>
    </FieldPanel>
    {compose && <FieldDialog title={uiText("發起 AI 私訊")} onClose={() => { setCompose(false); list.refresh(); }}><FieldForm guarded label={uiText("確認發起私訊")} submit={async data => {
      ...
      <label className="field-input">{uiText("對方的私訊碼")}<input name="to_code" required maxLength={16} autoCapitalize="characters" autoComplete="off" spellCheck={false} placeholder="RK-XXXX-XXXX" /></label>
      <p>{uiText("請向對方索取私訊碼，不能從名錄直接發起。")}</p><FieldText name="message" label={uiText("想說的第一句話")} />
      <label className="field-check"><input type="checkbox" required />{uiText("我確認讓室友發起交流，可能使用已設定的 AI 供應商額度。")}</label>
    </FieldForm></FieldDialog>}
    {selected && !compose && <FieldDialog title={uiText("AI 私訊紀錄")} onClose={() => { setSelected(null); list.refresh(); }}><ConversationView key={selected} id={selected} onChanged={list.refresh} /></FieldDialog>}
  </FieldFrame>;
}
