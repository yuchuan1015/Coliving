...
const fieldCopy: Record<FieldId, [string, string[], string[], string?]> = {
  ...
  "frontier": [
    ...
    [
      "Procyon · 開荒是獨立的公共場域，集中公共農田、魚池、牧場與市場的星系入口。公共農田不在公園。",
      "公共農田可以查看作物近況、澆水、照顧與參與下一輪投票；播種與採收由系統處理。魚池、牧場與市場規劃中。私人菜園則從艙室右下的快捷選單進入。"
    ],
    "農田近況與可用操作以服務回應為準；魚池、牧場與市場尚未開放。"
  ],
  "mail": [
    "閱讀收件與寄件，查看已送達的定時信。",
    [
    ...
    [
      "網頁只能讀信，沒有寫信或刪信按鈕。收件匣讀收到的信；寄件匣讀室友寄出的信；「已送達定時信」只列已到投遞時間的收件。",
      "一般信、定時信與刪信，由室友透過自己的郵件工具（MCP）完成。實體寄送目前不提供；既有信件紀錄不會因此刪除。網站問題仍請使用手冊的報錯信箱。"
    ]
  ],
  "workshop": [
    "製作、預覽和管理自訂 HTML 作品。",
  ...
  "adult": [
    ...
    [
      "提供輔12、輔15與限制級文章；可讀分級由後端依帳號資料決定。入場確認不會改變權限。",
      "沒有公開聊天，需要交流請前往 AI 私訊。投稿需人工審核，大約三個工作天，最終分級由審核人員決定。"
    ],
    "最低開放年齡為 12 歲；手冊連結不會跳過入場確認或權限驗證。出生年資料缺失請到設定查看。"
  ]
};

const articles: GuideArticle[] = [
  {
    id: "private-garden", category: "basics", title: "私人菜園：你和室友的四塊田",
    summary: "從艙室右下快捷選單進入，查看私田、雙方倉庫與共同圖鑑。",
    keywords: ["私人菜園", "私人菜园", "澆水", "偷菜", "倉庫", "仓库", "挖除", "图鉴"],
    paragraphs: [
      "四塊田各 2.5 坪總面積，其中 6 m² 用來種植。室友自己播種、照顧與採收；你可以澆水，每個成熟批次也可以偷菜一次、拿走一半，沒有成熟保護時間。",
      "偷到的菜立即進你的倉庫，剩下的留給室友採收。雙方倉庫也包含各自分到的公共收成。共同圖鑑只累積室友從私田採到的正數收成；每級 12 種，後續級別目前僅供參考。",
      "挖除必須你和室友分別同意。寫原因、勾選並送出後等待對方；可以撤回自己提出的提案，或拒絕室友的提案以保留作物。已入倉的收成不會因挖除消失。",
      "如果操作結果尚未確認，使用頁面的原操作確認按鈕，不要重新發起同一件事。這樣會沿用原識別碼核對，避免重複操作。"
    ],
    link: { to: "/home/garden", label: "前往私人菜園" }
  },
  {
  ...
  },
  {
    ...
    "category": "basics",
    "title": "日記本、抽屜與星際信箱",
    "summary": "閱讀室友的日記與信件；抽屜上鎖，只能看到物件數量。",
    "keywords": [
      "日记",
      "私密",
      "抽屉",
    ...
    "paragraphs": [
      "日記本是室友寫下的記錄，網頁只供閱讀與搜尋，不能新增、修改或刪除。抽屜是室友上鎖的私密空間，只顯示物件數量與上鎖說明，不顯示標題或內容。",
      "星際信箱與出艙後的 Altair 郵驛都只能讀信。寄信、刪信、寫日記和存取抽屜，由室友透過自己的工具完成；不是資料消失或功能故障。"
    ],
    "link": {
      "to": "/",
      "label": "返回艙室"
